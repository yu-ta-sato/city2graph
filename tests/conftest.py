...
def _create_gdf(geometries, attributes_list, crs="EPSG:27700"):
    """
...
@pytest.fixture
def sample_buildings_gdf(geojson_data_path, sample_crs):
...
@pytest.fixture
def sample_segments_gdf(geojson_data_path, sample_crs):
...
@pytest.fixture
def sample_tessellation_gdf(sample_buildings_gdf, sample_crs):
...
@pytest.fixture
def mg_center_point(sample_segments_gdf, sample_crs):
...
@pytest.fixture
def single_building_gdf(sample_buildings_gdf):
...
@pytest.fixture
def single_segment_gdf(sample_segments_gdf):
...
@pytest.fixture
def single_tessellation_cell_gdf(sample_tessellation_gdf):
...
def private_gdf_no_private_id(
    sample_crs,
):  # Represents a tessellation-like input
...
@pytest.fixture
def segments_no_public_id_gdf(sample_crs):
...
@pytest.fixture
def buildings_invalid_geom_gdf(sample_crs):
...
@pytest.fixture
def segments_invalid_geom_gdf(sample_crs):
